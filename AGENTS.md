...
| `src/ui.rs` | Pure rendering: `draw(&mut Frame, &App)`. Borderless, with a small margin: tabs (current session marked with a trailing `*`, pre-selected tab highlighted, no divider), a column header + the tree `List`, and a footer (help / filter input / transient status). Colors come from the `DIM`/`DIM_SEL`/`SELECT_BG` constants plus `cpu_style`/`mem_style`; helpers `format_bytes`, `truncate`. |
